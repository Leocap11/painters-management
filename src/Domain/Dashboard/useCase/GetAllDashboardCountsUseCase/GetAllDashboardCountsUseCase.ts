...
export class GetAllDashboardCountsUseCase {
  constructor(
    private readonly getCountSupplierUseCase: GetCountSupplierUseCase,
  ...
  async run(): Promise<DashboardCountsModel> {
    return {
      clients: await this.getCountClientUseCase.run(),
      materials: await this.getCountMaterialUseCase.run(),
      suppliers: await this.getCountSupplierUseCase.run(),
      workOrders: await this.getCountWorkOrderUseCase.run()
    };
  }
}
